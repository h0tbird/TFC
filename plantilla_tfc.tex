...
Para conseguir alta disponiblilidad (HA) en un cluster real serían necesarias 3 o 5 instancias de \emph{Mesos Master} (ver apartado 1.3.4) y 3 o 5 instancias de \emph{Zookeeper}. En este caso, para la prueba de concepto en esta maqueta es suficiente con una única instancia de cada uno. \emph{Zookeeper} y \emph{Etcd} (ver apartado 1.3.3.1) son componentes que ofrecen funcionalidades muy similares y a la larga \emph{Zookeeper} será reemplazado.
...
El primer test verifica que no exista ya una máquina virtual con el mismo identificador en el hipervisor local. El segundo test realiza una verificación DNS para asegurarse de que no existe ya otra máquina en la red con el mismo nombre. El tercer test verifica que no se supere el límite establecido de máquinas virtuales por hipervisor.
...
\emph{Marathon} [15] es un \emph{framework} de \emph{Mesos} especialmente diseñado para ejecutar aplicaciones que tienen una esperanza de vida larga como, por ejemplo, un servidor web \emph{Apache} o una base de datos de clave valor \emph{Redis}.
...
En este capítulo se detalla la plataforma de arranque. Esta plataforma se utiliza para instalar la plataforma de explotación descrita en el capítulo primero. Veremos las funciones de los distintos elementos que la componen, cómo configurarlos y también la manera de inspeccionar su estado.
...
\bibitem{item15}
Marathon
{\it https://mesosphere.github.io/marathon/}
